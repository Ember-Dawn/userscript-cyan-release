...
- **清空**：位于左侧，使用更宽、更醒目的垃圾桶图标。点击后显示脚本自己渲染的确认弹窗，确认后只删除代码块内容、保留代码块本身，并把编辑光标恢复到空代码块第一行。
...
- 删除后不主动清空浏览器 Selection；
- 等待 ProseMirror 完成 DOM 同步后，再把折叠光标放回当前代码块第一行；
- 最终保留空代码块，并允许用户直接继续输入。

光标恢复使用延后的浏览器渲染周期执行，避免刚设置的 Selection 被 ProseMirror 随后的 DOM 同步覆盖。如果浏览器或后续 NocoDB/Tiptap 版本不再接受这种编辑删除路径，应优先调整为宿主认可的 ProseMirror transaction，而不是退回直接改写编辑器 DOM。
