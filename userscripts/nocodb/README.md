...
`nocodb-folders.user.js` V11.1 使用紧凑的加号文件夹按钮、首次同步四分支决策、设置面板实时状态反馈、页面聚焦触发、仅前台低频检查、ETag 条件写入、冲突安全副本和每日快照实现自动多端同步。WebDAV 主文件仍采用完整 JSON 快照，但连续操作会被防抖合并，相同内容不会重复上传。展开状态和外观设置只保存在当前设备。
