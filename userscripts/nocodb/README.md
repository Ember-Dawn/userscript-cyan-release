...
| NocoDB 文件夹 | `nocodb-folders.user.js` | 为 NocoDB 表格提供文件夹式组织、排序和同步。 |
...
普通正文中的手动换行会导出为 Markdown 反斜杠硬换行，不再输出 HTML `<br>`。表格单元格内部没有通用的纯 Markdown 多行语法，因此其中的换行仍使用 `<br>`，以免破坏整张 Markdown 表格。
