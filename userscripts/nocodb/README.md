...
| NocoDB Rich Text 视觉样式增强 | `nocodb-richtext-style.user.js` | 为 H1-H6、加粗文字、`【xxx】` 和 `「xxx」` 提供显示层颜色增强，并增大普通顶层段落间距；列表/引用块等嵌套段落不受影响，纯空段落不额外放大。 |
...
- `nocodb-richtext-style.user.js` 只应修改显示层：标题/加粗和顶层段落间距使用 CSS，`【xxx】` / `「xxx」` 使用 CSS Custom Highlight；段落间距只作用于 ProseMirror 直接子级 `p`，纯空段落不额外增加 margin；不要向 ProseMirror 正文插入包装节点，也不要注册 ProseMirror plugin。
