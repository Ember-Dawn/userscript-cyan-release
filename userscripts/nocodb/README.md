...
| NocoDB Rich Text 视觉样式增强 | `nocodb-richtext-style.user.js` | 为 H1-H6、加粗文字、`【xxx】` 和 `「xxx」` 提供显示层颜色增强；顶层普通段落与有序/无序列表后统一使用 1.0em 间距，列表内部和纯空段落不额外放大。 |
...
- `nocodb-richtext-style.user.js` 只应修改显示层：标题/加粗和顶层块间距使用 CSS，`【xxx】` / `「xxx」` 使用 CSS Custom Highlight；块间距只作用于 ProseMirror 直接子级 `p` / `ol` / `ul`，列表内部 `li > p` 不额外放大，纯空段落不额外增加 margin；不要向 ProseMirror 正文插入包装节点，也不要注册 ProseMirror plugin。
