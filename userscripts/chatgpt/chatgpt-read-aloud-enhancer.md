...
## 4.0.6 Voice 模式误触发播放器修复

- 将“捕获页面媒体播放”和“允许激活朗读播放器”分离，避免把 ChatGPT Voice 模式或其他无关音频误认为消息朗读；
- 只有脚本刚发起“朗读 / 重播”的 `opening` 会话，或当前播放器已经绑定的同一音频继续播放 / 结束后重播时，才允许激活悬浮播放器；
- 未经脚本请求的新 `<audio>` 即使调用 `play()` 也不会替换当前朗读音频或弹出播放器；
- 保留 detached audio、MediaSource/AAC 捕获以及 HTTP / Blob / MSE 三种 MP3 下载路径，不改变已验证可用的下载行为。

## 4.0.7 操作栏按钮视觉顺序兼容修复

- 修复 ChatGPT 操作栏改用反向 Flex 排列后，自定义“朗读 / 重播”按钮虽然仍位于 DOM 末尾、视觉上却跑到第一位的问题；
- 根据操作组当前的 `flex-direction` 放置按钮：普通 `row` 保持追加到末尾，`row-reverse` 则放到 DOM 首位，从而继续保持视觉上的最后一个位置；
- 已存在的自定义按钮也会按当前布局重新归位，避免页面局部重渲染后顺序漂移；
- 不改变 Voice 模式过滤、朗读 / 重播触发、播放器、MediaSource/AAC 捕获和 MP3 下载逻辑。

## 4.0.8 Voice 退出后的按钮顺序稳定修复

- 修复进入并退出 Voice 模式后，ChatGPT 操作栏在过渡阶段重建 / 切换 Flex 布局，导致“朗读 / 重播”按钮再次漂到视觉第一位的问题；
- 在现有 DOM 首尾校正之外，为自定义按钮设置与当前 `flex-direction` 对应的极值 `order`，让 Flex 布局稳定后仍保持视觉最后一个；
- 页面 `class` / `style` 或子节点变化后会进行防抖复查，并在短暂布局过渡结束后再做一次收敛复查，覆盖 Voice 模式退出后的延迟样式恢复；
- 不使用持续轮询，不改变 Voice 模式过滤、朗读 / 重播、播放器、MediaSource/AAC 捕获和 MP3 下载逻辑。
